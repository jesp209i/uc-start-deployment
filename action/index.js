...
// src/ucStartDeploymentApi.ts
var import_http_client = __toESM(require_lib());
async function startDeployment(callUrl, apiKey) {
  const headers = {
...
async function run() {
  try {
    ...
    (0, import_core.info)("Deployment accepted and queued to start shortly");
  } catch (error) {
    (0, import_core.warning)(`Error: ${error}`);
    (0, import_core.setFailed)("Got an error while trying to start a deployment");
  }
}
run();
